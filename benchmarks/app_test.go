...
func SetupWithGenesisAccounts(b testing.TB, db dbm.DB, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *app.WasmApp {
	...
	wasmApp.Commit()
	wasmApp.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: wasmApp.LastBlockHeight() + 1}})
...
func InitializeWasmApp(b testing.TB, db dbm.DB, numAccounts int) AppInfo {
	...
	wasmApp.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: height, Time: time.Now()}})
